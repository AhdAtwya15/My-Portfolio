...
const Navbar = () => {
  ...
  return (
    <nav className="max-w-7xl mx-auto w-full py-4 px-6 sm:px-10 flex justify-between items-center  fixed top-0 left-0 right-0 z-50 transition-colors duration-300">
      ...
      >
          ...
          {NAV_LINKS.map((link, i) => (
            ...
            >
              ...
              <a
                ...
                onClick={closeMenu}
                className="text-white text-3xl font-bold group-hover:text-teal-500 transition-colors duration-200">{link}
              </a>
        ...
        <button onClick={closeMenu} className="cursor-pointer absolute top-8 right-10">
          <IoCloseOutline className="w-7 h-7 text-white hover:text-slate-400 transition-colors duration-300" />
        </button>

      </div>

      <a href="#contact" className='inline-block bg-linear-to-r from-[#1CD8D2] via-[#00bf8f] to-[#302b63]  hover:from-[#2DA696] hover:via-[#1FAF98] hover:to-[#1FAF98] hover:scale-105  text-white px-5 py-3 rounded-full transition-all duration-300'>
        Hire Me
      </a>
    </nav>
  )
